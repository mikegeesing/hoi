                ...
                <form method="post" action="{$WEB_ROOT}/cart.php" id="frmDomainChecker" {if isset($RSThemes['addonSettings']['tld_cycle_switcher']) && $RSThemes['addonSettings']['tld_cycle_switcher'] == 'true'}data-show-tld-cycle-switcher {if isset($smarty.get.period)}data-period="{$smarty.get.period}"{/if}{/if}>
                    ...
                    <div class="domain-search-input search-group search-group-lg">
                        <div class="search-field">
                            <input class="form-control"  type="text" name="domain" placeholder="{$LANG.findyourdomain}" value="{$lookupTerm}" id="inputDomain" data-toggle="tooltip" data-delay="100" data-placement="top" data-trigger="manual" title="{lang key='orderForm.domainOrKeyword'}" />
                            <div class="search-field-icon"><i class="lm lm-search"></i></div>
                        </div>
                        <div class="search-group-btn">
                            <button class="btn btn-primary{if $searchBoxStyle == 'primary'}-faded{/if} domain-check-availability {if $captcha}{$captcha->getButtonClass($captchaForm)}{/if}" type="submit" id="btnCheckAvailability">
