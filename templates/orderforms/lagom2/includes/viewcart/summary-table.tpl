                            ...
                            <div class="row" data-input-number>
                                <div class="prod-name {if $showqtyoptions || $showAddonQtyOptions}col-6{else}col-7{/if}" data-content="{$LANG.orderproduct}">
                                    <div class="cart-item-title">
                                        <span class="cart-item-title-main">{if isset($RSThemes['pages'][$templatefile]) && $RSThemes['pages'][$templatefile]['config']['removeProductGroupName'] != "1"}{$product.productinfo.groupname} - {/if}{$product.productinfo.name}</span>
                                        {assign var="hideServerFields" value=false}
                                        {if $RSThemes['addonSettings']['hide_password_fields'] != "displayed" && isset($RSThemes['addonSettings']['hide_hostname_on_checkout']) && $RSThemes['addonSettings']['hide_hostname_on_checkout'] == "1"}
                                            {assign var="ns_groups" value=","|explode:$RSThemes['addonSettings']['hide_password_fields']}
                                            {if $product.productinfo.gid|in_array:$ns_groups}
                                                {assign var="hideServerFields" value=true}
                                            {/if}
                                        {/if}
                                        {if $product.domain && !$hideServerFields}
