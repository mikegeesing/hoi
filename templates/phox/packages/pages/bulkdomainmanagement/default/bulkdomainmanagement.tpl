<form method="post" action="{$smarty.server.PHP_SELF}?action=bulkdomain" class="form-horizontal">
    ...
    {if $update eq "nameservers"}
        ...
        <p>
            <label class="full control-label"><input type="radio" class="radio inline" name="nschoice" value="default"
                    onclick="disableFields('domnsinputs',true)" checked /> {$LANG.nschoicedefault}</label>
            <br />
            <label class="full control-label"><input type="radio" class="radio inline" name="nschoice" value="custom"
                    onclick="disableFields('domnsinputs','')" checked /> {$LANG.nschoicecustom}</label>
        </p>
    ...
    {elseif $update eq "autorenew"}
        ...
        <p align="center">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp; <input type="submit" name="disable" value="{$LANG.domainsautorenewdisable}"
    ...
    {elseif $update eq "reglock"}
        ...
        <p align="center">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp; <input type="submit" name="disable" value="{$LANG.domainreglockdisable}"
                class="btn btn-danger btn-large" />
        </p>
    ...
    {elseif $update eq "contactinfo"}
        ...
        {foreach from=$contactdetails name=contactdetails key=contactdetail item=values}
            ...
            <h3>
                <a
                    name="{$contactdetail}"></a>{$contactdetail}</strong>{if $smarty.foreach.contactdetails.first}
                        {foreach from=$contactdetails name=contactsx key=contactdetailx item=valuesx}
                            {if !$smarty.foreach.contactsx.first}
                            - <a href="clientarea.php?action=bulkdomain#{$contactdetailx}">{$LANG.jumpto}
                            {$contactdetailx}</a>{/if}
                            {/foreach}
                        {else} - <a
                        href="clientarea.php?action=bulkdomain#">{$LANG.top}</a>{/if}
                </h3>
                ...
                <p>
                    <label class="full control-label"><input type="radio" class="radio inline" name="wc[{$contactdetail}]"
                            id="{$contactdetail}1" value="contact" onclick="usedefaultwhois(id)" />
                        {$LANG.domaincontactusexisting}</label>
                </p>
                ...
                <fieldset class="onecol" id="{$contactdetail}defaultwhois">
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <select class="{$contactdetail}defaultwhois" name="sel[{$contactdetail}]" id="{$contactdetail}3"
                                onclick="usedefaultwhois(id)">
                ...
                <p>
                    <label class="full control-label"><input type="radio" class="radio inline" name="wc[{$contactdetail}]"
                            id="{$contactdetail}2" value="custom" onclick="usecustomwhois(id)" checked />
                        {$LANG.domaincontactusecustom}</label>
                </p>
                ...
                <fieldset class="onecol" id="{$contactdetail}defaultwhois">
                    ...
                    {foreach key=name item=value from=$values}
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" name="contactdetails[{$contactdetail}][{$name}]" value="{$value}" size="30"
                                    class="{$contactdetail}customwhois" />
                            </div>
                        </div>
                    {/foreach}
