...
{if !$invalidTicketId}
  <div class="wdes-view-ticket panel panel-info panel-collapsable{if !$postingReply} panel-collapsed{/if} hidden-print">
    ...
    <div class="panel-body{if !$postingReply} panel-body-collapsed{/if}">
      ...
      <form method="post" action="{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;postreply=true"
        enctype="multipart/form-data" role="form" id="frmReply">

        <div class="row">
          <div class="form-group col-sm-6">
            <label for="inputName">{$LANG.supportticketsclientname}</label>
            <input class="form-control" type="text" name="replyname" id="inputName" value="{$replyname}" {if $loggedin}
              disabled="disabled" {/if}>
          </div>
          <div class="form-group col-sm-6">
            <label for="inputEmail">{$LANG.supportticketsclientemail}</label>
            <input class="form-control" type="text" name="replyemail" id="inputEmail" value="{$replyemail}" {if $loggedin}
              disabled="disabled" {/if}>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <textarea name="replymessage" id="inputMessage" rows="12" class="form-control markdown-editor"
            data-auto-save-name="ctr{$tid}">{$replymessage}</textarea>
        </div>
        ...
        <div class="row form-group">
          ...
          <div class="col-xs-12 ticket-attachments-message text-muted">
            ...
            ({lang key="maxFileSize" fileSize="$uploadMaxFileSize"})
          </div>
        </div>
  ...
  <div class="wdes-tickets-replies-list">
    {foreach $descreplies as $reply}
      <div class="ticket-reply markdown-content{if $reply.admin} staff{/if}">
        <div class="wdes-phox-ticket-heading">
          <div class="user">
            {* Profile Picture *}
            {if $reply.admin}
              <div class="wdes-phox-profile-picture wdes-phox-profile-picture--staff">
                <img src="{$WEB_ROOT}/templates/{$template}/img/logo-dark-icon.png" alt="Avatar for {$reply.requestor.name}" />
              </div>
            {else}
              <div class="wdes-phox-profile-picture">
                <img src="{$loggedinuser.gravatar}" alt="Avatar for {$reply.requestor.name}" />
              </div>
            {/if}
            <span class="name">
              <span>{$reply.requestor.name}</span>
              <span class="label wdes-ticket-reply-role">
