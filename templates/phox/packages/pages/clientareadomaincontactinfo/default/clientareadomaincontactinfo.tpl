...
{if $domainInformation && !$pending && $domainInformation->getIsIrtpEnabled() && $domainInformation->isContactChangePending()}
    {if $domainInformation->getPendingSuspension()}
        {include file="$template/includes/alert.tpl" type="warning" msg="<strong>{$LANG.domains.verificationRequired}</strong><br>{$LANG.domains.newRegistration}"
        textcenter=true}
    {else}
        {include file="$template/includes/alert.tpl" type="info" msg="<strong>{$LANG.domains.contactChangePending}</strong><br>{$LANG.domains.contactsChanged}"
        textcenter=true}
    {/if}
{/if}
    ...
    <div class="row">
        ...
        {foreach from=$contactdetails name=contactdetails key=contactdetail item=values}
            ...
            <div class="col-md-6">
                ...
                <div class="radio">
                    <label>
                        <input type="radio" name="wc[{$contactdetail}]" id="{$contactdetail}1" value="contact"
                            onclick="useDefaultWhois(this.id)" />
                ...
                <div class="row">
                    <div class="col-xs-offset-1 col-xs-10">
                        <div class="form-group">
                            ...
                            <select id="{$contactdetail}3" class="form-control {$contactdetail}defaultwhois"
                                name="sel[{$contactdetail}]" disabled>
                ...
                <div class="radio">
                    <label>
                        <input type="radio" name="wc[{$contactdetail}]" id="{$contactdetail}2" value="custom"
                            onclick="useCustomWhois(this.id)" checked />
                ...
                {foreach key=name item=value from=$values}
                    <div class="form-group">
                        ...
                        <input type="text" name="contactdetails[{$contactdetail}][{$name}]" value="{$value}"
...
{if $domainInformation && $irtpFields}
    ...
    <div class="modal fade" id="modalIRTPConfirmation" role="dialog" aria-labelledby="IRTPConfirmationLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content panel panel-primary">
                ...
                <div id="modalIRTPConfirmationFooter" class="modal-footer panel-footer">
                    <button type="button" id="IRTPConfirmation-Submit" class="btn btn-primary"
                        onclick="irtpSubmit();return false;">
