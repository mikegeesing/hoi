        ...
        <div class="panel panel-default panel-form">
            ...
            <form id="frmManagePaymentMethod" class="frm-credit-card-input" role="form" method="post" action="{if $editMode}{routePath('account-paymentmethods-save', $payMethod->id)}{else}{routePath('account-paymentmethods-add')}{/if}">
                ...
                <div class="form-group">
                    ...
                    <div class="radio">
                        {if $enabledTypes['tokenGateways']}
                            {foreach $tokenGateways as $tokenGateway}
                                <label class="radio-inline icheck-label">
                                    <input type="radio" class="icheck-button" name="type" value="token_{$tokenGateway}"
                                        data-tokenised="true" data-gateway="{$tokenGateway}"
                                        {if $editMode && $payMethod->isCreditCard() && $payMethod->gateway_name == $tokenGateway}
                                            checked{/if}{if $editMode} disabled{/if}>
                        ...
                        {if $enabledTypes['localCreditCard']}
                            <label class="radio-inline icheck-label">
                                <input type="radio" class="icheck-button" name="type" value="localcard"
                                    {if ($editMode && $payMethod->isCreditCard() && !$payMethod->isTokenised()) || (!$editMode && $paymentMethodType != 'bankacct')}
                                        checked{/if}{if $editMode} disabled{/if}>
                        ...
                        {if $enabledTypes['bankAccount']}
                            <label class="radio-inline icheck-label">
                                <input type="radio" class="icheck-button" name="type" value="bankacct"
                                    {if ($editMode && !$payMethod->isCreditCard()) || ($paymentMethodType == 'bankacct')}
                                        checked{/if}{if $editMode} disabled{/if}>
                    ...
                    <div class="cc-details">
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <input type="tel"
                                        class="form-control cc-number-field {$creditCard->getCardType()|strtolower}"
                                        id="inputCardNumber" name="ccnumber" autocomplete="cc-number"
                                        value="{$creditCard->getMaskedCardNumber()}" {if !$creditCardNumberFieldEnabled}
                                        disabled{/if} aria-describedby="cc-type"
                        ...
                        {if $startDateEnabled}
                            <div class="form-group">
                                ...
                                <div>
                                    <div class="row">
                                        <div class="col-sm-4">
                                            <input type="tel" class="form-control" id="inputCardStart" name="ccstart" autocomplete="off"
                                                value="{if $creditCard->getStartDate()}{$creditCard->getStartDate()->format('m / y')}{/if}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                <div class="row">
                                    <div class="col-sm-4">
                                        <input type="tel" class="form-control" id="inputCardExpiry" name="ccexpiry"
                                            autocomplete="cc-exp"
                                            value="{if $creditCard->getExpiryDate()}{$creditCard->getExpiryDate()->format('m / y')}{/if}"
                                            {if !$creditCardExpiryFieldEnabled} disabled{/if}>
                                    </div>
                                </div>
                        ...
                        {if $creditCardCvcFieldEnabled}
                            <div class="form-group">
                                ...
                                <div>
                                    <input type="tel" class="form-control input-inline input-inline-100" id="inputCardCvc"
                                        name="cardcvv" autocomplete="off">
                                    <button id="cvvWhereLink" type="button" class="btn btn-link" data-toggle="popover"
                ...
                <div class="fieldgroup-bankaccount{if $remoteUpdate || ($editMode && $payMethod->isCreditCard()) || ($paymentMethodType != 'bankacct' && !$editMode)} hidden{/if}">
                    <div class="form-group">
                        <label for="inputBankAcctType" class="control-label">{$LANG.paymentMethodsManage.accountType}</label>
                        <div>
                            <label class="radio-inline icheck-label">
                                <input type="radio" class="icheck-button" name="bankaccttype" id="inputBankAcctType"
                                    value="Checking"
                                    {if !$bankAccount->getAccountType() || $bankAccount->getAccountType() == 'Checking'}
                                    checked{/if}>
                                {$LANG.paymentMethodsManage.checking}
                            </label>
                            <label class="radio-inline icheck-label">
                                <input type="radio" class="icheck-button" name="bankaccttype" value="Savings"
                                    {if $bankAccount->getAccountType() == 'Savings'} checked{/if}>
                    ...
                    <div class="form-group">
                        <label for="inputBankAcctHolderName"
                            class="control-label">{$LANG.paymentMethodsManage.accountHolderName}</label>
                        <div>
                            <input type="tel" class="form-control" id="inputBankAcctHolderName" name="bankacctholdername"
                                autocomplete="off" value="{$bankAccount->getAccountHolderName()}">
                            <span class="field-error-msg">{$LANG.paymentMethods.fieldRequired}</span>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="inputBankName" class="control-label">{$LANG.paymentMethodsManage.bankName}</label>
                        <div>
                            <input type="tel" class="form-control" id="inputBankName" name="bankname" autocomplete="off"
                                value="{$bankAccount->getBankName()}">
                            <span class="field-error-msg">{$LANG.paymentMethods.fieldRequired}</span>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="inputBankRoutingNum" class="control-label">{$LANG.paymentMethodsManage.sortCodeRoutingNumber}</label>
                        <div>
                            <input type="tel" class="form-control" id="inputBankRoutingNum" name="bankroutingnum" autocomplete="off"
                                value="{$bankAccount->getRoutingNumber()}">
                            <span class="field-error-msg">{$LANG.paymentMethodsManage.routingNumberNotValid}</span>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="inputBankAcctNum" class="control-label">{$LANG.paymentMethodsManage.accountNumber}</label>
                        <div>
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <a href="#" class="btn btn-default btn-sm" data-toggle="modal"
                                data-target="#modalBillingAddress">{$LANG.paymentMethodsManage.addNewAddress}</a>
                        </div>
                    </div>
                    <div class="form-group submit-container">
                        <div class="col-sm-8 col-sm-offset-4">
                            <button type="submit" name="submit" id="btnSubmit"
                                class="btn btn-primary">{$LANG.clientareasavechanges}</button>
            ...
            <div class="modal fade" id="modalBillingAddress" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <form id="billingContactForm" action="{routePath('account-paymentmethods-billing-contacts-create')}"
                            data-role="json-form">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="companyname" id="inputCompanyName"
                                                value="{$contactcompanyname}" class="form-control">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="country" id="inputCountry">
                                                {foreach $countries as $countryCode => $countryName}
                                                    <option value="{$countryCode}" {if ($countryCode == $clientCountry)}
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-default"
                                    data-dismiss="modal">{$LANG.paymentMethods.close}</button>
        ...
        jQuery(document).on('click', '.frm-credit-card-input button[type="submit"]', function(e) {
            ...
            if (checkedInput.val() === 'bankacct') {
                ...
                if (
                    ...
                    (!$.payment.validateCardNumber(cardNumber.val()) || cardNumber.hasClass(
                        'unsupported'))
                ) {
                ...
                if (ccExpiryFieldEnabled && !$.payment.validateCardExpiry(ccForm.find(
                        '#inputCardExpiry').payment('cardExpiryVal'))) {
                    ccForm.find('#inputCardExpiry').showInputError();
                    e.preventDefault();
                }
                if (ccCvcFieldEnabled && !$.payment.validateCardCVC(ccForm.find('#inputCardCvc').val(),
                        cardType)) {
                ...
                WHMCS.http.jqClient.jsonPost({
                    ...
                    success: function(response) {
                        ...
                        if (response.remoteInputForm) {
                            jQuery('#tokenGatewayRemoteInputOutput').html(response
                                .remoteInputForm);
                            jQuery('#tokenGatewayRemoteInputOutput').find('form:first')
                                .attr('target', 'ccframe');
                            setTimeout(
                                "autoSubmitFormByContainer('tokenGatewayRemoteInputOutput')",
                                1000);
                            ...
                            jQuery('.fieldgroup-creditcard').removeClass('hidden').show(
                                'fast',
                                function() {
                                    jQuery('#tokenGatewayAssistedOutput').html(response
                                        .assistedOutput);
                                    if (!paymentInitSingleton.has(module)) {
                                        WHMCS.payment.event.gatewayInit(
                                            whmcsPaymentModuleMetadata, module,
                                            element);
                                        WHMCS.payment.event.gatewayOptionInit(
                                            whmcsPaymentModuleMetadata, module,
                                            element);
                                        paymentInitSingleton.set(module, true);
                                    }
                                    WHMCS.payment.event.gatewaySelected(
                                        whmcsPaymentModuleMetadata, module, element);
                                });
