{if file_exists("templates/$template/packages/overwrites/oauth/login-twofactorauth.tpl")}
    {include file="{$template}/packages/overwrites/oauth/login-twofactorauth.tpl"}
{else}
            ...
            <div id="loginWithBackupCode" {if !$backupcode} class="hidden" {/if}>
                ...
                <div class="action-buttons">
                    <button type="button" class="btn btn-default" id="btnCancel"
                        onclick="jQuery('#frmCancelLogin').submit()">
                        {lang key='cancel'}
                    </button>
                </div>
            </div>
            ...
            <div id="loginWithSecondFactor" {if $backupcode} class="hidden" {/if}>
                ...
                <div class="action-buttons">
                    <div class="pull-left text-left small">
                        ...
                        <a href="#"
                            onclick="jQuery('#loginWithSecondFactor').hide();jQuery('#loginWithBackupCode').removeClass('hidden').show();">{$LANG.twofaloginusingbackupcode}</a>
                    </div>
                    <button type="button" class="btn btn-default" id="btnCancel"
                        onclick="jQuery('#frmCancelLogin').submit()">
                        {lang key='cancel'}
