{if file_exists("templates/$template/packages/overwrites/oauth/layout.tpl")}
    {include file="{$template}/packages/overwrites/oauth/layout.tpl"}
{else}
    <!DOCTYPE html>
    <html lang="en">
                ...
                <div class="pull-right text-right">
                    {if $loggedin}
                        <form method="post" action="{$issuerurl}oauth/authorize.php" id="frmLogout">
                            ...
                            <p>
                                {lang key='oauth.currentlyLoggedInAs' firstName=$userInfo.firstName lastName=$userInfo.lastName}{if $userInfo.clientName}
                                ({$userInfo.clientName}){/if}.
