{if file_exists("templates/$template/packages/overwrites/payment/card/select.tpl")}
    {include file="{$template}/packages/overwrites/payment/card/select.tpl"}
{else}
    {if count($existingCards) > 0}
        <div class="three-column-grid">
            {foreach $existingCards as $cardInfo}
                ...
                <div class="paymethod-info" data-paymethod-id="{$cardInfo.paymethodid}">
                    <input id="existingCard{$cardInfo.paymethodid}" type="radio" name="ccinfo" class="existing-card icheck-button"
                        data-billing-contact-id="{$cardInfo.billingcontactid}"
                        {if $cardOnFile && !$payMethodExpired && $payMethodId eq $cardInfo.paymethodid}
                            {assign "preselectedBillingContactId" {$cardInfo.billingcontactid}} checked="checked"
                        data-loaded-paymethod="true" {elseif ($cardOnFile && $payMethodExpired) || !$cardOnFile} disabled="disabled"
                            {/if} {if !$hasRemoteInput}onclick="hideNewCardInputFields();" {/if} value="{$cardInfo.paymethodid}">
                    </div>
                    <div class="paymethod-info" data-paymethod-id="{$cardInfo.paymethodid}">
        ...
        <div class="paymethod-info">
            <label>
                <input id="newCCInfo" type="radio" class="icheck-button" name="ccinfo" value="new"
                    {if $payMethodId eq "new" || !$cardOnFile} checked{/if} /> {$LANG.creditcardenternewcard}
            </label>
        </div>
    {/if}