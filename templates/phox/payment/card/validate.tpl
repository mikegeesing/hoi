{if file_exists("templates/$template/packages/overwrites/payment/card/validate.tpl")}
    {include file="{$template}/packages/overwrites/payment/card/validate.tpl"}
{else}
    {if !$hasRemoteInput}
        <script>
            ...
            function validateCreditCardInput(e) {
                ...
                if (newOrExisting === 'new') {
                    ...
                    if (!jQuery.payment.validateCardNumber(ccForm.find('#inputCardNumber').val()) || cardNumber.hasClass(
                            'unsupported')) {
