{if file_exists("templates/$template/packages/overwrites/store/sitebuilder/upgrade.tpl")}
    {include file="{$template}/packages/overwrites/store/sitebuilder/upgrade.tpl"}
{else}
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="promo-wrapper">
                                {if count($siteBuilderServices) > 0}
                                    ...
                                    {else}
                                        ...
                                        <form method="post" action="{routePath('cart-site-builder-upgrade-order')}">
                                            <select name="service"
                                                class="form-control sitebuilder-service-select{if count($siteBuilderServices) == 1} hidden{/if}">
                                ...
                                {else}
                                    ...
                                    <p><a
                                            href="logout.php?redirect=cart-site-builder-upgrade">{lang key="store.siteBuilder.upgrade.switchUser"}</a>
                                    </p>
                                {/if}
                            </div>
