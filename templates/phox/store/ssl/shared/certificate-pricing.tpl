{if file_exists("templates/$template/packages/overwrites/store/ssl/shared/certificate-pricing.tpl")}
    {include file="{$template}/packages/overwrites/store/ssl/shared/certificate-pricing.tpl"}
{else}
    <div class="content-block certificate-options {$type}">
        <div class="container">
            ...
            <div class="row">
                <div
                    class="{if count($certificates.$type) == 1}col-md-6 col-md-offset-3{elseif count($certificates.$type) == 2}col-md-10 col-md-offset-1{else}col-sm-12{/if}">
                    <div class="row row-pricing-table">
                        <div
                            class="col-sm-{if count($certificates.$type) == 1}6{elseif count($certificates.$type) == 2}4{else}3{/if} sidebar hidden-xs">
                        ...
                        {if count($certificates.$type) > 0}
                            {foreach $certificates.$type as $product}
                                <div
                                    class="col-sm-{if count($certificates.$type) == 1}6{elseif count($certificates.$type) == 2}4{else}3{/if}">
                                    <div class="header">
                                    ...
                                    <form method="post" action="{routePath('cart-order')}">
                                        ...
                                        <button type="submit"
                                            class="btn btn-block">{lang key='store.ssl.landingPage.buyNow'}</button>
                                    </form>
                                </div>
                            {/foreach}
                        {else}
                    ...
                    <div class="row">
                        <div
                            class="{if count($certificates.$type) == 1}col-sm-6 col-sm-offset-6{elseif count($certificates.$type) == 2}col-sm-8 col-sm-offset-4{else}col-sm-9 col-sm-offset-3{/if} text-center">
                            <a href="{routePath('store-product-group', $routePathSlug)}#helpmechoose"
