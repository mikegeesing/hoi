{if file_exists("templates/$template/packages/overwrites/store/promos/slimupsell.tpl")}
    {include file="{$template}/packages/overwrites/store/promos/slimupsell.tpl"}
{else}
            ...
            <div class="content">
                ...
                <form method="post" action="{routePath('store-order')}">
                    ...
                    <button type="submit" class="btn btn-success">
                        {$promotion->getCta()} {$product->name} {if $product->isFree()}
                            {lang key="orderfree"}
                        {else}from just
                        {$product->pricing()->best()->breakdownPrice()}{/if}
                    </button>
