{if file_exists("templates/$template/packages/overwrites/store/weebly/upgrade.tpl")}
    {include file="{$template}/packages/overwrites/store/weebly/upgrade.tpl"}
{else}
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="promo-wrapper">
                                {if count($weeblyServices) > 0}
                                    ...
                                    {else}
                                        ...
                                        <form method="post" action="{routePath('cart-weebly-upgrade-order')}">
                                            <select name="service"
                                                class="form-control weebly-service-select{if count($weeblyServices) == 1} hidden{/if}">
                                ...
                                {else}
                                    ...
                                    <p><a
                                            href="logout.php?redirect=cart-weebly-upgrade">{lang key="store.websiteBuilder.upgrade.switchUser"}</a>
                                    </p>
                                {/if}
                            </div>
