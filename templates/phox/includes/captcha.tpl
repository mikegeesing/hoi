{if file_exists("templates/$template/packages/overwrites/includes/captcha.tpl")}
    {include file="{$template}/packages/overwrites/includes/captcha.tpl"}
{else}
                ...
                {elseif !$captcha->recaptcha->isEnabled()}
                    <div class="col-md-8 col-md-offset-2 col-xs-10 col-xs-offset-1">
                        <div id="default-captcha-domainchecker"
                            class="{if $filename == 'domainchecker'}input-group input-group-box {/if}text-center">
                            ...
                            <div class="col-xs-6">
                                <input id="inputCaptcha" type="text" name="code" maxlength="6"
                                    class="form-control {if $filename == 'register'}pull-left{/if}" data-toggle="tooltip"
                                    data-placement="right" data-trigger="manual" title="{lang key='orderForm.required'}" />
                            </div>
                        </div>
                    </div>
                {/if}
