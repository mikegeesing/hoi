{if file_exists("templates/$template/packages/overwrites/includes/pwstrength.tpl")}
    {include file="{$template}/packages/overwrites/includes/pwstrength.tpl"}
{else}
    <br />
    ...
    {if file_exists("templates/$template/includes/alert.tpl")}
        {include file="$template/includes/alert.tpl" type="info" msg="{$LANG.passwordtips}" additionalClasses="alert-have-list"}
    {elseif file_exists("templates/phox/includes/alert.tpl")}
        {include file="phox/includes/alert.tpl" type="info" msg="{$LANG.passwordtips}" additionalClasses="alert-have-list"}
    {/if}
    ...
    <script type="text/javascript">
        jQuery("#inputNewPassword1").keyup(function() {
            ...
            jQuery("#inputNewPassword1").next('.form-control-feedback').removeClass(
                'glyphicon-remove glyphicon-warning-sign glyphicon-ok');
            jQuery("#passwordStrengthBar .progress-bar").removeClass(
                    "progress-bar-danger progress-bar-warning progress-bar-success").css("width", pwstrength + "%")
        ...
        function validatePassword2() {
            ...
            if (password2 && password1 !== password2) {
                $newPassword2.removeClass('has-success')
                    .addClass('has-error');
                jQuery("#inputNewPassword2").next('.form-control-feedback').removeClass('glyphicon-ok').addClass(
                    'glyphicon-remove');
                ...
                if (password2) {
                    ...
                    jQuery("#inputNewPassword2").next('.form-control-feedback').removeClass('glyphicon-remove').addClass(
                        'glyphicon-ok');
                    {if !isset($noDisable)}jQuery('.main-content input[type="submit"]').removeAttr('disabled');{/if}
                } else {
                    $newPassword2.removeClass('has-error has-success');
                    jQuery("#inputNewPassword2").next('.form-control-feedback').removeClass(
