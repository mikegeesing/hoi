<div class="panel panel--collapse">
    <div class="collapse-toggle">
        <h6 class="top__title">
            ...
            {if isset($tooltips.settings.general.gravatar.title)}
                {if isset($tooltips.settings.general.gravatar.url) && $tooltips.settings.general.gravatar.url != ""}
                    {assign var="popoverFooter" value="<a class='btn btn--secondary btn--xs' href='{$tooltips.settings.general.gravatar.url}' target='_blank'>Learn More</a>"}
                {else}
                ...
                {include 
                    ...
                    popoverBody="{$tooltips.settings.general.gravatar.title}"
    ...
    <div class="collapse {if $settings['show_gravatar_image'] == "displayed"} show {/if}" id="gravatar-settings">
        <div class="form-group m-b-0x p-3x">
            <label class="form-label text-default">
                ...
                {if isset($tooltips.settings.general.gravatar_placeholder.title)}
                    {if isset($tooltips.settings.general.gravatar_placeholder.url) && $tooltips.settings.general.gravatar_placeholder.url != ""}
                        {assign var="popoverFooter" value="<a class='btn btn--secondary btn--xs' href='{$tooltips.settings.general.gravatar_placeholder.url}' target='_blank'>Learn More</a>"}
                    {else}
                        {assign var="popoverFooter" value=false}
                    {/if}
                    {include 
                        file="adminarea/includes/helpers/popover.tpl" 
                        popoverClasses="notification__popover"
                        popoverBody="{$tooltips.settings.general.gravatar_placeholder.title}"
                        popoverFooter="{$popoverFooter}"
                    }
                {/if}
            </label>
